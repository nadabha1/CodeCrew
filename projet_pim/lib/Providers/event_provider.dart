...
class EventProvider with ChangeNotifier {
  ...
  Future<void> fetchAllEvents() async {
    ...
    notifyListeners();
    try {
      final response = await http.get(Uri.parse('$baseUrl/events/all')); // Match backend findAllEvents
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        _events = data.map((json) => Event.fromJson(json, userId)).toList();
      } else {
        throw Exception('Failed to load all events: Status ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> createEvent(String userId, String title, String description, DateTime date, String location, int joinPrice) async {
    _isLoading = true;
    notifyListeners();
    try {
